...
const chairSlice = createSlice({
  ...
  initialState,
  reducers: {
    themVaoMangDanhSachGhe: (state, action) => {
      let index = state.arrGheBanChon.findIndex((value, index) => {
        return value.soGhe == action.payload.soGhe;
      });
      if (index != -1) {
        state.arrGheBanChon.splice(index, 1);
      } else {
        state.arrGheBanChon.push(action.payload);
      }
    },
  },
});

export const { themVaoMangDanhSachGhe } = chairSlice.actions;
